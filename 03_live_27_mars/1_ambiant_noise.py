...
pad2.fadein(16, fvol=.4)
pad2.thick_thin=linvar([0,1,.4,.8,0,.6], PRand(2,24), start=Clock.mod(4))

pad1.fade(16, fvol=.6)
pad2.vol = linvar(P[.75,1,.7,1.3,.6,1.2]*.5, PRand(8,32,seed=3)[:12], start=Clock.mod(4))
...
pad2.vol = linvar(P[.8,1,.7,1.3,.6,1.2]*.5, PRand(8,32,seed=3)[:12], start=Clock.mod(4))
pad1.vol = linvar([.8,1,.7,1.3,.6,1.2], PRand(8,32,seed=2)[:12], start=Clock.mod(4))
...
ticks.amplify = linvar(P[1,.2,.7,.3,1.3]*3, PRand(4,16), start=Clock.mod(4))

Scale.default = Scale.minor
lonesynth >> lone1([0], oct=7, dur=3, vol=.7)
...
kick2.fade(16, fvol=.5)
kick1.amplify=var([0,.5],2),
wbass >> wobble3([0], oct=3, dur=24, fm_tone=0)
wbass.fadein(24,fvol=.64)
wbass.span(srot(24))

pad1.fade(16, fvol=.6)
pad2.fade(16, fvol=.1)

wbass.vol = .8*linvar([.8,1,.7,1.3,.6,1.2], PRand(8,32,seed=3)[:12], start=Clock.mod(4))
wbass.span(srot(24))
