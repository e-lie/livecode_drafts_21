...
kick1 >> play("<v..v..v.><x..x..x.>", dur=.25, amp=1, lpf=600, hpf=60, output=12)
...
acidbass.envmod = linvar([0,.9,0], [8,8,inf], start=Clock.mod(4))
...
kick1 >> play("v..vb.v.", dur=.25, amp=.7, lpf=600, hpf=60, output=12)
...
bongo1 >> play(".c.....c", amp=2, dur=.25, rate=[2.5,2.5,3], room2=.4).stop()#.fadein(26)
...
kick1 >> play("X..X..X.", dur=.25, amp=1, lpf=800)
...
bongo2 >> play(".c..", amp=3, dur=1/3, rate=5, room2=1)
bongo2.amplify = linvar(P[.4,1,.7,1.3,.6,1.2]*.75, PRand(8,32,seed=5)[:12], start=Clock.mod(4))

hihat1.rate=1
bongo3 >> play("c..", amp=3, dur=1/3, rate=7, room2=1)
...
kick2 >> play("V..V..V.", dur=.25, amp=1, lpf=900, output=12)
...
bongo1 >> play(".c.c.c.cc", amp=1.7, dur=1/3, rate=4, room2=1)
